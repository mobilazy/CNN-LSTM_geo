...
def create_comprehensive_collector_analysis(test_df, predictions, targets, feature_cols, config_analysis):
    """Create comprehensive collector configuration analysis with wider actual vs predicted plot."""
    ...
    # Create figure with custom layout: large main plot + 2 side panels
    fig = plt.figure(figsize=(20, 10))
    gs = fig.add_gridspec(2, 4, hspace=0.3, wspace=0.3, width_ratios=[3, 3, 1, 1])
    
    # Main plot: Actual vs Predicted (spans 2 rows, 2 columns)
    ...
    for bhe_type in plot_data['bhe_type'].unique():
        ...
        ax_main.plot(type_data['Timestamp'], type_data['actual_temp'], 
                    color=color, alpha=0.7, linewidth=1.5, label=f'Actual {label}')
        
        # Plot predicted temperatures
        ax_main.plot(type_data['Timestamp'], type_data['predicted_temp'], 
                    color=color, alpha=0.8, linewidth=1.5, linestyle='--', 
                    label=f'Predicted {label}')
    
    ax_main.set_xlabel('Time', fontsize=14, fontweight='bold')
    ax_main.set_ylabel('Outlet Temperature (°C)', fontsize=14, fontweight='bold')
    ax_main.set_title('Actual vs Predicted Temperature for Wells with Different Collector Configuration', 
    ...
    # Calculate model accuracy metrics for each collector type
    collector_metrics = {}
    ...
    # Side Plot 1: Model Accuracy (MAE/RMSE)
    ax_acc = fig.add_subplot(gs[0, 2:])
    
    types = list(collector_metrics.keys())
    mae_vals = [collector_metrics[t]['mae'] for t in types]
    rmse_vals = [collector_metrics[t]['rmse'] for t in types]
    ...
    bars1 = ax_acc.bar(x_pos - width/2, mae_vals, width, label='MAE', 
                      color='#4CAF50', alpha=0.8)
    bars2 = ax_acc.bar(x_pos + width/2, rmse_vals, width, label='RMSE', 
                      color='#FF9800', alpha=0.8)
    
    ax_acc.set_xlabel('Collector Type', fontsize=12, fontweight='bold')
    ax_acc.set_ylabel('Prediction Error (°C)', fontsize=12, fontweight='bold')
    ax_acc.set_title('Model Accuracy\nby Collector Type', fontsize=14, fontweight='bold')
    ax_acc.set_xticks(x_pos)
    ax_acc.set_xticklabels([label.replace(' ', '\n') for label in type_labels], fontsize=10)
    ...
    for bar, val in zip(bars1, mae_vals):
        ...
        ax_acc.text(bar.get_x() + bar.get_width()/2., height + 0.002,
    ...
    # Side Plot 2: Relative Performance Benefits (using Single U45mm as reference)
    ax_perf = fig.add_subplot(gs[1, 2:])
    
    # Calculate average return temperatures for relative performance
    collector_avg_temps = {}
    for bhe_type in plot_data['bhe_type'].unique():
        type_data = plot_data[plot_data['bhe_type'] == bhe_type]
        collector_avg_temps[bhe_type] = type_data['actual_temp'].mean()
    
    # Use Single U45mm as reference (baseline)
    baseline_temp = collector_avg_temps.get('single_u45mm', 0)
    
    temp_benefits = []
    for bhe_type in types:
        benefit = collector_avg_temps[bhe_type] - baseline_temp
        temp_benefits.append(benefit)
    
    bars3 = ax_perf.bar(range(len(types)), temp_benefits,
                       color=type_colors, alpha=0.8)
    
    ax_perf.set_xlabel('Collector Type', fontsize=12, fontweight='bold')
    ax_perf.set_ylabel('Relative Performance\nBenefit (°C)', fontsize=12, fontweight='bold')
    ax_perf.set_title('Relative Performance Benefits\n(vs Single U45mm)', fontsize=14, fontweight='bold')
    ax_perf.set_xticks(range(len(types)))
    ax_perf.set_xticklabels([label.replace(' ', '\n') for label in type_labels], fontsize=10)
    ax_perf.grid(True, alpha=0.3, axis='y')
    ax_perf.axhline(y=0, color='gray', linestyle='--', alpha=0.7)
    
    # Add value labels on performance bars
    for i, (bar, benefit) in enumerate(zip(bars3, temp_benefits)):
        height = bar.get_height()
        sign = '+' if benefit >= 0 else ''
        ax_perf.text(bar.get_x() + bar.get_width()/2., 
    ...
    print(f"\nModel Accuracy by Collector Type:")
    for i, bhe_type in enumerate(types):
        label = type_labels[i]
        metrics = collector_metrics[bhe_type]
        benefit = temp_benefits[i]
        
        print(f"  {label}:")
        print(f"    MAE: {metrics['mae']:.4f}°C")
        print(f"    RMSE: {metrics['rmse']:.4f}°C")
        print(f"    Test samples: {metrics['count']:,}")
        print(f"    Relative benefit vs Single U45mm: {benefit:+.3f}°C")
...
def create_collector_performance_analysis(all_data_clean):
    ...
    for bhe_type, data in all_data_clean.items():
        ...
        # Apply smoothing (20-point moving average)
        window_size = 20
...
def create_raw_data_collector_analysis(complete_field, double_u45, muovi_ellipse):
    ...
    # Apply smoothing (20-point moving averages)
    window_size = 20
...
def main():
    ...
    print("Creating comprehensive collector analysis...")
    comprehensive_plot_path = create_comprehensive_collector_analysis(test_df, predictions, targets, feature_cols, config_analysis)    # Save results
    results = {
        'model_performance': {
